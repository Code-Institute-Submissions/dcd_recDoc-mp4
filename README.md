...
database.  
#### Front end ####
It was decided to use simple colors that would be consistent across all elements and pages. Icons are also
incorporated into the forms to supplement field titles and placeholder text. Headings in the main are bold 
and simple, but direct to mitigate any ambiguity. Button links are consistent and framed in rectangles implying
functionality. Background colours are white so as not to detract the user from focusing on the elements and 
content.  
...
DC.js charting is the preferred method to present the summary data, rather than using mongo db.aggregation 
pipeline method, in conjunction with matplotlib.  While not responsive on smaller screen sizes dc.js is an 
